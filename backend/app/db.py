...
# The reserved AI opponent, present so vs-AI games have a real user row to score.
BOT_USERNAME = "playerAI"
...
def ensure_bot_user(password_hash: str, avatar: str | None) -> None:
    if get_user(BOT_USERNAME) is None:
        create_user(BOT_USERNAME, password_hash, avatar)
...
def leaderboard() -> list[dict]:
    conn = connect()
    cur = conn.execute(
        "SELECT username, avatar, wins, losses FROM users WHERE username != ? ORDER BY wins DESC, losses ASC",
        (BOT_USERNAME,),
    )
    return [dict(r) for r in cur.fetchall()]
